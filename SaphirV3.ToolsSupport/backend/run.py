...
def check_web_ressource(server, username,password,database,lien_crm,serveur_batch,nom_ressource="gs2e_fonctionsGenerales"):
    ...
    try:            
        ...
        # elif 
        return {'bd':bd,'crm':lien,'service':service, 'portion':portion,'status':200, 'catalogue':catalogue, 'message': message}
    except:  # noqa: E722
        # print (f"Pas de ressource {nom_ressource} trouvée sur le catalogue {database}")
        return {'message':f"Pas de ressource {nom_ressource} trouvée sur le catalogue {database}",'status':400,'catalogue':database,}
    # finally:
    #     conn.close()
    ...
    try:
        ...
        for key, value in filtre_hostname_serveur.items():
            ...
            if value is not None and value != 'null' and server_available(value) == True:
                ...
                etat_serveur.append({'serveur':value,'role':key,'etat':'En ligne','os_name':os_name,'os_version':os_version})
            else:
                etat_serveur.append({'serveur':value,'role':key,'etat':'Pas disponible','os_name':'N/A','os_version':'N/A'})
...
def check_crm_avaibility():
    ...
    try:
        ...
        if response.status_code == 200:
            return jsonify({'status':200,'message':f"Dynamics CRM est disponible à {crm_url}. l'utilisateur {username} s'est connecté avec succès",'response':True})
        else:
...
def check_server_sql_avaibility():
    ...
    try:
        ...
        connection.close()
        return jsonify({'status':200,'message':f"L'instance '{instance_sql}' est disponible.",'response':True})
        # return True
    except pyodbc.Error as e:
        # Si la connexion échoue, afficher l'erreur
        # print(f"Échec de la connexion à l'instance '{instance_sql}'. Erreur : {e}")
        return jsonify({'status':400,'message':f"Échec de la connexion à l'instance '{instance_sql}'. Erreur : {e}",'response':False})
...
def get_windows_service():
    ...
    if role == 'batch':
        ...
        liste_services = [
                            "MSCRMAsyncService",  # Service de traitement asynchrone MSCRMAsyncService
                            "MSCRMAsyncService$maintenance", # Service de traitement Bac à sable MSCRMSandboxService
                            "MSCRMSandboxService", # Service de traitement Bac à sable MSCRMSandboxService
                        ]
        print('backend')
        for service in liste_services:
            service = check_service_status_remote_powershell(server, service, username, password)
            # print(service)
            services.append({
                'service': service,
                'status':state
